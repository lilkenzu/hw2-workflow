# Процесс и автоматизация — отчёт по курсам ЦБ РФ
### Агент: Инженер Процессов (Process Architect)

Зона ответственности этого документа: **что делать с находками аудита** и **как получить весь отчёт одной командой в следующий раз**. Сами находки (дубликаты/пропуски/аномалии/сверка) — см. `audit-findings.md`, бизнес-риски — зона Агента 2.

---

## 1. Немедленные fix-действия по находкам аудита

| # | Находка аудита | Fix-действие |
|---|---|---|
| 1 | Ответ XML в windows-1251, не UTF-8 | Всегда декодировать явно `raw.decode('cp1251')` до парсинга XML. Никогда не полагаться на дефолтную кодировку `requests`/`urlopen`. |
| 2 | Десятичный разделитель — запятая | Перед `float()` всегда `.replace(',', '.')`. Обернуть парсинг числа в try/except с явным логом даты и валюты при сбое, а не тихим `ValueError`/пропуском строки. |
| 3 | `Value` дан за `Nominal` единиц, для USD/EUR/CNY сейчас =1, но это не гарантия | Всегда делить `Value / Nominal`, без исключений и без "оптимизации под текущий набор валют". Это правило, а не частный случай. |
| 4 | Пропуски по понедельникам — норма (курс пятницы действует Сб/Вс/Пн) | Зашить в проверку пропусков явное правило: понедельник, идущий сразу за присутствующей пятницей, — ожидаемый пропуск, не аномалия. Флагить как проблему только пропуски, не покрытые этим правилом. |
| 5 | Скачки >2% день-к-дню (USD 20→21.08, EUR 27→28.08) | Не «чинить» данные — это зона оценки риска Агента 2. Со стороны процесса: закрепить порог 2% как параметр (не хардкод в проде), чтобы его можно было пересматривать без правки кода. |
| 6 | Сверка со вторым источником вручную, разовая | Сделать сверку обязательным автоматическим шагом каждого прогона с порогом допустимого расхождения (напр. 1%) и явным статусом OK/ПРЕВЫШЕНИЕ в отчёте, а не отдельным ручным действием после факта. |
| 7 | Весь пайплайн выполнялся вручную, шаг за шагом | Собрать в один воспроизводимый скрипт (раздел 3) с одной точкой входа и ненулевым exit-code при находках — пригодно для cron/CI, не только для ручного запуска. |

---

## 2. Дизайн Workflow и обоснование выбора формы

**Выбор: bash+python скрипт, а не промпт-шаблон для агента.**

Обоснование одной фразой: весь пайплайн — детерминированная ETL-задача с чёткими числовыми порогами (encoding, разделитель, деление на Nominal, %-пороги дубликатов/пропусков/аномалий/расхождения), где не требуется суждение — а значит нужна не переменная интерпретация LLM при каждом запуске, а фиксированная логика, которую можно положить в cron и которая каждый раз даёт побитово одинаковый результат на одних и тех же входных данных.

Архитектура одной командой:

```
run_report.sh  →  cbr_report.py
                     ├─ fetch_dynamic()      # сбор: XML_dynamic.asp по каждой валюте за период
                     ├─ audit_currency()     # аудит: дубликаты / пропуски (с правилом понедельника) / аномалии >2%
                     ├─ fetch_crosscheck()   # сверка: open.er-api.com, кросс-курс EUR/CNY через RUB
                     └─ build_report()       # сборка: report.md для руководителя
```

Выходной код процесса: `0` — данных без находок, `2` — есть находки, требующие внимания (необъяснённые пропуски или аномалии), любой другой ненулевой код — сбой пайплайна (сеть, парсинг, неизвестная валюта). Это разделение специально сделано для cron/CI: `0` — тишина, `2` — приложить отчёт к письму, иначе — алерт на сбой инфраструктуры.

---

## 3. Готовый Workflow-артефакт

Два файла: `cbr_report.py` (логика) и `run_report.sh` (точка входа). Зависимости — только стандартная библиотека Python 3.9+, ничего дополнительно ставить не нужно.
...
case $STATUS in
  ...
  *) echo "СБОЙ пайплайна (см. вывод выше), отчёт мог не сформироваться" >&2; exit "$STATUS" ;;
esac
```
...
### Автоматизация по расписанию (пример cron)

```
# Ежемесячно, 1-го числа в 09:00 — отчёт за прошлый месяц, лог в файл
0 9 1 * * /path/to/workflow/run_report.sh --last-month >> /path/to/workflow/logs/cron.log 2>&1
```

Скрипт возвращает `2`, если есть необъяснённые пропуски или аномалии — это можно использовать как условие отправки алерта (например, `if [ $? -eq 2 ]; then mail -s "Отчёт ЦБ: есть находки" ...`), не открывая отчёт вручную каждый раз.

---

## 4. Резюме для итогового отчёта руководителю (3–5 пунктов)

- **Три точки отказа парсера устранены системно**: явная кодировка cp1251, замена `,`→`.`, обязательное деление `Value/Nominal` без исключений для любой валюты — зашиты в код, а не в память оператора.
- **Пропуски по понедельникам формализованы как бизнес-правило**, а не ручное объяснение постфактум: скрипт больше не будет ложно флагить их как сбой.
- **Сверка со вторым источником (open.er-api.com) стала обязательным автоматическим шагом** каждого прогона с порогом 1% и явным статусом OK/ПРЕВЫШЕНИЕ в самом отчёте, а не отдельной ручной операцией.
- **Весь пайплайн (сбор → парсинг → аудит → сверка → отчёт) сведён к одной команде** (`run_report.sh --last-month`), готов к постановке в cron с кодом возврата `2` как триггером алерта при находках.
- Пороги аномалии (2%) и допустимого расхождения со вторым источником (1%) вынесены в параметры скрипта — их можно пересматривать по решению владельца процесса без правки логики.
